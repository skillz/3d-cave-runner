...
public sealed class Obstacle : MonoBehaviour
{
    ...
    float SpeedChange = 0.8f; //How much rhe player's speed is affected. A value below 1 will slow the player down, while a value above 1 will speed him up
    private float SpeedChangeModifier = 0.5f;
    float TurnSpeedChange = 0.1f; //How much the Turning speed is affected. A value lower than 1 will make rotation slower
    Vector3 Bounce = new Vector3(0, 10, 0); //How much to bounce the player and in what direction

    float JumpChange = 1; //How much the jump power is affected. Lower than 1 means his jump is weaker
    float JumpChangeTime = 0; //How long to keep this change

    string HitAnimation = "Lava"; //What player animation to run when hitting this obstacle
    float HitAnimationTime = 0; //How long to keep this animation

    bool AnimateObstacle = false; //Should we animate the obstacle itself when hitting the player. If the obstacle has an animation, it will be played if this value is set to true

    private bool HitPlayer = false; //Did we hit the player?
    AudioClip HitSound; //The sound to be played when hitting the player

    bool StickToPlayer = false; //Do we stick to the player? If set to true, the obstacle will stick to the player for some time and then disappear, creating a Disperse effect in its position
    private bool StuckToPlayer = false; //Are we stuck to the player now?
    float StickTime = 0; //How long should the obstacle stick to the player
    Transform DisperseEffect; //The effect to be created after the stick time has passed

    bool LookAtPlayer = false; //Should the obstacle look at the player

    Vector3 Rotation = new Vector3(270, 0, 0); //Set an initial rotation for the obstacle, in case it turns out to be wrongly rotated
    Vector3 Offset = new Vector3(0, 0, 0); //Offset the object in any direction at when created

    PlayerControls kPlayerControls;
